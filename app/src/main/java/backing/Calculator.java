...
import static backing.Constants.BRACKET;
import static backing.Constants.HARF_ROUND_UP;
import static backing.Constants.OPERATOR;
import static backing.Constants.SINGLE_OP;
import static backing.Constants.WORD_CONSTANT;
import static backing.Constants.WORD_NEED_ONE;
import static backing.Constants.WORD_NEED_TWO;
...
public class Calculator {
    ...
    private BigDecimal calc(String data) {
        ...
        for (int i = 0; i < Constants.CONVERT_FROM.length; i++) {
            if (data.contains(Constants.CONVERT_FROM[i])) {
                data = data.replace(Constants.CONVERT_FROM[i], Constants.CONVERT_TO[i]);
            }
        }
